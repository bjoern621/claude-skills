...
Install and configuration answer a question this reader has not asked yet, so the argument comes first and it runs on code.
...
The comparison is the argument, so steps 3 and 4 perform one task twice and hold everything else identical between them.
...
A README addresses somebody deciding, so "you" and "your" are correct on this page and the second-person ban in [reference.md](reference.md) does not reach it.
...
A capability list is a contract, so every line on it is checked against the code before it ships.
